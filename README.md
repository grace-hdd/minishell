...
- **Heredocs are collected before execution.** Every `<<` in the line is read
...
- **Generating test cases.** Asking for lists of edge cases (nest quotes, empty variables,
  several redirections on once command, etc) to run agains our shell. Every expected result was checked by running the same input in Bash.
